...
Given('there are already {int} tasks in status {string}') do |count, status|
  ...
  if project.tasks.where(status: status.parameterize.underscore).count < count
    (count - project.tasks.where(status: status.parameterize.underscore).count).times do |i|
      project.tasks.create!(title: "Pre-existing Task #{i+1}", status: status.parameterize.underscore)
    end
  end
  expect(project.tasks.where(status: status.parameterize.underscore).count).to eq(count)
end
...
Given('I should see a task titled {string} in status {string}') do |title, status|
  ...
  current_project.tasks.find_or_create_by!(title: title) do |t|
    t.status = status.parameterize.underscore
  end
end
...
Then('{string} should remain in {string}') do |task_title, expected_status|
  ...
  expect(task.status).to eq(expected_status.parameterize.underscore),
    "Expected status '#{expected_status.parameterize.underscore}' but was '#{task.status}'. "\
  ...
  within("##{expected_status.parameterize.dasherize}-column") do
